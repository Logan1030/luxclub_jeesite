...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/luxclub/cardBalance/">会员卡余额列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="memberInfo" action="${ctx}/luxclub/cardBalance/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>过期日期</th>
				<th>备注信息</th>
				<shiro:hasPermission name="luxclub:cardBalance:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="memberInfo">
			<tr>
				<td><a href="${ctx}/luxclub/cardBalance/feeList?memberCardno=${memberInfo.memberCardno}">
				...
				<td>
					<c:if test="${memberInfo.state != 3 && memberInfo.state != 4}">
						<a href="${ctx}/luxclub/cardBalance/rechargeForm?id=${memberInfo.id}">充值</a>
						<a href="${ctx}/luxclub/cardBalance/deductForm?id=${memberInfo.id}">扣费</a>
					</c:if>
				</td>
				</shiro:hasPermission>
			</tr>
