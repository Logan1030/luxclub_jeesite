...
public class Category extends TreeEntity<Category> {
	...
	private static final long serialVersionUID = -7715547544074015568L;
	...
	public static void sortList(List<Category> list, List<Category> sourcelist, Long parentId) {
		for (int i = 0; i < sourcelist.size(); i++) {
			Category category = sourcelist.get(i);
			if (category.getParent() != null && category.getParent().getId() != null && category.getParent().getId().equals(parentId)) {
				list.add(category);
				// 判断是否还有子节点, 有则继续获取子节点
				for (int j = 0; j < sourcelist.size(); j++) {
					Category child = sourcelist.get(j);
					if (child.getParent() != null && child.getParent().getId() != null
							&& child.getParent().getId().equals(category.getId())) {
						sortList(list, sourcelist, category.getId());
						break;
					}
				}
			}
